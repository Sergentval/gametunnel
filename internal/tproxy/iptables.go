...
// iptablesManager is the legacy iptables-based manager (kept for fallback).
type iptablesManager struct {
	ipt *iptables.IPTables
}

// newIPTablesManager creates a TPROXY Manager backed by go-iptables.
func newIPTablesManager() (Manager, error) {
...
func (m *iptablesManager) AddRule(_ string, port int, mark string) error {
...
func (m *iptablesManager) RemoveRule(_ string, port int, mark string) error {
	portStr := strconv.Itoa(port)
	for _, proto := range []string{"tcp", "udp"} {
		rulespec := []string{
			"-p", proto,
...
func (m *iptablesManager) EnsurePolicyRouting(_ string, _ int) error { return nil }

// CleanupPolicyRouting is a no-op placeholder; actual cleanup is handled by
// routing.Manager via CleanupTPROXYRouting.
func (m *iptablesManager) CleanupPolicyRouting(_ string, _ int) error { return nil }
